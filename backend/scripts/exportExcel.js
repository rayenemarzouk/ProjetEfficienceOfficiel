...
async function main() {
  ...
  XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(repRows), 'Rapports');
  ...
  XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(userRows), 'Utilisateurs');
...
async function main() {
  ...
  const realRows = realisations.map(r => ({
    ...
    Mois:               formatMois(r.mois),
    Mois_Brut:          r.mois || '',
    'CA Facturé (€)':   fmtEur(r.montantFacture),
    'CA Encaissé (€)':  fmtEur(r.montantEncaisse),
    'Taux Encaissement (%)': r.montantFacture > 0
      ? ((r.montantEncaisse / r.montantFacture) * 100).toFixed(1)
      : '0.0',
    'Nb Patients':      r.nbPatients || 0,
    'Panier Moyen (€)': r.nbPatients > 0
      ? fmtEur(r.montantFacture / r.nbPatients)
      : '0.00',
    'Nouveaux Dossiers': r.nouveauxDossiers || 0,
    'Règlements Année':  r.reglementsPourAnnee || 0,
  }));
  const wsReal = XLSX.utils.json_to_sheet(realRows);
  XLSX.utils.book_append_sheet(wb, wsReal, 'CA & Réalisations');
  ...
  const rdvRows = rdvs.map(r => ({
    ...
    Mois_Brut:          r.mois || '',
    'Nb RDV':           r.nbRdv || 0,
    'Nb Patients RDV':  r.nbPatients || 0,
    'Nouveaux Patients': r.nbNouveauxPatients || 0,
    'RDV Honorés':      r.rdvHonores || 0,
    'RDV Manqués':      r.rdvManques || 0,
    'Annulations':      r.annulations || 0,
    'Reports':          r.reports || 0,
    'RDV Importants':   r.rdvImportants || 0,
    'Durée Totale (min)': r.dureeTotaleRdv || 0,
    'Durée Moy (min)':  r.dureeMoyennePrevue || 0,
    'RDV / Jour':       r.rdvParJour || 0,
  }));
  const wsRdv = XLSX.utils.json_to_sheet(rdvRows);
  XLSX.utils.book_append_sheet(wb, wsRdv, 'Rendez-vous');
  ...
  const hRows = heures.map(r => ({
    ...
    Mois:              formatMois(r.mois),
    Mois_Brut:         r.mois || '',
    'Jours Ouverts':   r.joursOuverts || 0,
    'Heures Travaillées (min)': r.nbHeures || 0,
    'Heures Travaillées (h)': ((r.nbHeures || 0) / 60).toFixed(1),
  }));
  const wsH = XLSX.utils.json_to_sheet(hRows);
  XLSX.utils.book_append_sheet(wb, wsH, 'Jours & Heures');
  ...
  const devisRows = devis.map(r => ({
    ...
    Mois:                    formatMois(r.mois),
    Mois_Brut:               r.mois || '',
    'Nb Devis':              r.nbDevis || 0,
    'Nb Acceptés':           r.nbDevisAcceptes || 0,
    'Taux Acceptation (%)':  r.nbDevis > 0
      ? ((r.nbDevisAcceptes / r.nbDevis) * 100).toFixed(1)
      : '0.0',
    'Montant Proposé (€)':   fmtEur(r.montantPropositions),
    'Montant Accepté (€)':   fmtEur(r.montantAccepte),
    'Montant Réalisé (€)':   fmtEur(r.montantTotalRealise),
  }));
  const wsDevis = XLSX.utils.json_to_sheet(devisRows);
  ...
  const encRows = encours.map(r => ({
    Praticien:                   r.praticien || '',
    'Montant Total À Facturer (€)': fmtEur(r.montantTotalAFacturer),
    'Patients En Cours':          r.patientsEnCours || 0,
    'Rentabilité Horaire (€/h)':  fmtEur(r.rentabiliteHoraire),
    'Date Mise à Jour':           r.updatedAt ? new Date(r.updatedAt).toLocaleDateString('fr-FR') : '',
  }));
  const wsEnc = XLSX.utils.json_to_sheet(encRows);
  XLSX.utils.book_append_sheet(wb, wsEnc, 'Encours');

  // ── 6. Synthèse par Praticien ────────────────────────────────────────────
  const synth = {};
  for (const r of realisations) {
    if (!synth[r.praticien]) synth[r.praticien] = { praticien: r.praticien, caFacture: 0, caEncaisse: 0, nbPatients: 0, mois: new Set() };
    synth[r.praticien].caFacture  += r.montantFacture  || 0;
    synth[r.praticien].caEncaisse += r.montantEncaisse || 0;
    synth[r.praticien].nbPatients += r.nbPatients      || 0;
    synth[r.praticien].mois.add(String(r.mois).substring(0, 6));
  ...
  const rdvSynth = {};
  for (const r of rdvs) {
    if (!rdvSynth[r.praticien]) rdvSynth[r.praticien] = { nbRdv: 0, manques: 0 };
    rdvSynth[r.praticien].nbRdv    += r.nbRdv    || 0;
    rdvSynth[r.praticien].manques  += (r.rdvManques || 0) + (r.annulations || 0);
  }
  const synthRows = Object.values(synth).sort((a, b) => a.praticien.localeCompare(b.praticien)).map(s => {
    const rdv = rdvSynth[s.praticien] || {};
    return {
      Praticien:                s.praticien,
      'Nb Mois de Données':     s.mois.size,
      'CA Total Facturé (€)':   fmtEur(s.caFacture),
      'CA Total Encaissé (€)':  fmtEur(s.caEncaisse),
      'Taux Encaissement (%)':  s.caFacture > 0 ? ((s.caEncaisse / s.caFacture) * 100).toFixed(1) : '0.0',
      'Total Patients':         s.nbPatients,
      'Panier Moyen (€)':       s.nbPatients > 0 ? fmtEur(s.caFacture / s.nbPatients) : '0.00',
      'Total RDV':              rdv.nbRdv || 0,
      'Total Absences/Annul.':  rdv.manques || 0,
      'Taux Absence (%)':       rdv.nbRdv > 0 ? ((rdv.manques / rdv.nbRdv) * 100).toFixed(1) : '0.0',
    };
  });
  const wsSynth = XLSX.utils.json_to_sheet(synthRows);
  XLSX.utils.book_append_sheet(wb, wsSynth, 'Synthèse Praticiens');
